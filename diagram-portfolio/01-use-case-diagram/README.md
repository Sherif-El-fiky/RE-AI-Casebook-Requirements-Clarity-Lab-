...
The functional scope of the system from the outside: who interacts with it (actors) and what they can accomplish (use cases), bounded by the system boundary.

**Actors**
- **Patient** — primary actor, initiates `Search Availability`, `Book Appointment`, `Reschedule Appointment`, `Cancel Appointment`, `Check In`
- **Clinic Staff** — secondary human actor, connected to `Manage Provider Schedule`, `Book Appointment` (booking on a patient's behalf — walk-in or phone), and `Check In` (front-desk check-in)
...
A generic "Manage Provider Schedule" use case is doing too much work — in a real elicitation session I'd split it into at least "Set Availability" and "Block Time Off," because they have different triggers, different actors involved, and different edge cases (a blocked slot with an existing booking is a conflict that needs its own resolution rule). I kept it merged here to keep the diagram legible at portfolio scale, but flagging that trade-off explicitly is itself part of the RE judgment this repo is meant to demonstrate.
